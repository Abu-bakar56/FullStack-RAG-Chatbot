...
# from core.fuse_client import langfuse_client
# from services.auth import register_user, login_user
# from services.chat import save_chat_message, get_chat_history, get_user_threads, create_thread
# from core.models import AgentState
# from fastapi.middleware.cors import CORSMiddleware
...
# # ------------------ AUTH ------------------ #
# async def get_current_user(token: str = Depends(oauth2_scheme)):
#     # In real production validate JWT
#     return token  
...
# @app.post("/register")
# async def register(user: UserRegister):
#   with langfuse_client.start_as_current_generation(name="user_registration") as trace:
#     try:
#         user_id = register_user(user.username, user.email, user.password)
#         if not user_id:
#             raise HTTPException(status_code=400, detail="Registration failed")
...
# @app.post("/login")
# async def login(user: UserLogin):
#   with langfuse_client.start_as_current_generation(name="user_login") as trace:
#     try:
#         user_id = login_user(user.email, user.password)
#         if not user_id:
#             raise HTTPException(status_code=401, detail="Invalid credentials")
...
# @app.post("/chat")
# async def chat(request: ChatRequest, user_id: str = Depends(get_current_user)):
#     with langfuse_client.start_as_current_generation(name="chat_interaction") as trace:
#         trace.update(inputs={"query": request.query, "user_id": user_id})
...
# @app.get("/threads")
# async def get_threads(user_id: str = Depends(get_current_user)):
#     with langfuse_client.start_as_current_generation(name="get_threads") as trace:
#         threads = get_user_threads(user_id)
#         trace.update(outputs={"threads": threads}, status="SUCCESS")
#     return {"threads": threads}
...
# @app.get("/thread/{thread_id}")
# async def get_thread(thread_id: str, user_id: str = Depends(get_current_user)):
#   with langfuse_client.start_as_current_generation(name="get_thread_history") as trace:
#     history = get_chat_history(thread_id, user_id)
#     trace.update(outputs={"history": history}, status="SUCCESS")
#     return {"history": history}

...
# Define the FastAPI server URL (use environment variable or default to local)
...
def main():
    ...
    if not st.session_state.user_id:
        ...
        if st.session_state.show_register_form:
            ...
            with st.form("register_form"):
                ...
                if submit:
                    if username and email and password:
                        try:
                            response = requests.post(f"{SERVER_URL}/auth/register", json={"username": username, "email": email, "password": password})
                            if response.status_code == 200:
                                data = response.json()
                                st.session_state.user_id = data.get("user_id")
                                st.session_state.thread_id = create_thread(st.session_state.user_id)
        ...
        if user_input and isinstance(user_input, str) and user_input.strip():
            ...
            def stream_response():
                try:
                    response = requests.post(f"{SERVER_URL}/search/process", json=initial_state.dict())
                    if response.status_code == 200:
...
def create_thread(user_id):
    try:
        response = requests.post(f"{SERVER_URL}/chat/threads", json={"user_id": user_id})
        if response.status_code == 200:
            return response.json().get("thread_id")
        else:
            st.error("Failed to create new chat thread.")
            return None
    except requests.RequestException as e:
        st.error(f"Error connecting to server: {e}")
        return None
...
def get_user_threads(user_id):
    try:
        response = requests.get(f"{SERVER_URL}/chat/threads/{user_id}")
        if response.status_code == 200:
...
def get_chat_history(thread_id, user_id):
    try:
        response = requests.get(f"{SERVER_URL}/chat/history/{thread_id}/{user_id}")
        if response.status_code == 200:
...
def save_chat_message(thread_id, user_id, message):
    try:
        response = requests.post(f"{SERVER_URL}/chat/messages", json={"thread_id": thread_id, "user_id": user_id, "message": message})
        if response.status_code != 200:
            st.error("Failed to save chat message.")
    except requests.RequestException as e:
        st.error(f"Error connecting to server: {e}")
